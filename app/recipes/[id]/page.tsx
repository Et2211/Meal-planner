    ...
    <div className="min-h-screen bg-stone-50">
      <header className="sticky top-0 z-10 bg-white border-b border-stone-200">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href="/recipes" className="p-2 -ml-2 text-stone-500 hover:text-stone-700 hover:bg-stone-100 rounded-lg transition">
            <ArrowLeft size={18} />
          </Link>
          <span className="font-semibold text-stone-900 truncate">{r.title}</span>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-8">
          <h1 className="text-2xl font-bold text-stone-900">{r.title}</h1>
          <a
            href={r.source_url}
            target="_blank"
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <section>
            <h2 className="text-lg font-semibold text-stone-900 mb-3">
              Ingredients
              <span className="text-stone-400 font-normal text-sm ml-2">({r.ingredients.length})</span>
            </h2>
            <ul className="space-y-2">
              {r.ingredients.map((ing, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-stone-700">
                  <span className="w-1.5 h-1.5 rounded-full bg-brand-400 flex-shrink-0 mt-1.5" />
                  {formatShoppingItem({ name: ing.name, quantity: ing.quantity, unit: ing.unit, entries: [ing.raw] })}
                </li>
              ))}
            </ul>
          </section>
          ...
          <section>
            <h2 className="text-lg font-semibold text-stone-900 mb-3">
              Instructions
              <span className="text-stone-400 font-normal text-sm ml-2">({r.instructions.length} steps)</span>
            </h2>
